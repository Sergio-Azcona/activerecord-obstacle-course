...
# ----------------------
  ...
  it '9. finds orders for a user' do
    ...
    orders_of_user_3 = @user_3.orders  #using Model's relationship
    # ------------------------------------------------------------
  ...
  it '12. should return all items except items 2, 5 and 6' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    items = Item.where.not(id: items_not_included) #this works
    # items = Item.where.not(id: items_not_included.map { |item| item.id}) #Brian solution
    # items = Item.where.not(id: items_not_included.map {&:id}) #Brian solution; means same as above

    # require 'pry';binding.pry
    # items = Item.where.not(id: items_not_included).where(id: expected_result)
    # ------------------------------------------------------------
  ...
  it "13. groups an order's items by name" do
    ...
    # grouped_items = Order.all.joins(:items).select(:name)
    # ------------------------------------------------------------
